...
// node_modules/angular-resizable-element/fesm2015/angular-resizable-element.mjs
...
var ResizableDirective = class {
  ...
  constructor(platformId, renderer, elm, zone) {
    ...
    this.mouseup = new import_rxjs.Subject();
    this.mousedown = new import_rxjs.Subject();
    this.mousemove = new import_rxjs.Subject();
   ...
   */
  ngOnInit() {
    const mousedown$ = (0, import_rxjs.merge)(this.pointerEventListeners.pointerDown, this.mousedown);
    const mousemove$ = (0, import_rxjs.merge)(this.pointerEventListeners.pointerMove, this.mousemove).pipe((0, import_operators.tap)(({
      event
    }) => {
    ...
    const mouseup$ = (0, import_rxjs.merge)(this.pointerEventListeners.pointerUp, this.mouseup);
    let currentResize;
    ...
    const mousedrag = mousedown$.pipe((0, import_operators.mergeMap)((startCoords) => {
      ...
      return (0, import_rxjs.merge)(mousemove$.pipe((0, import_operators.take)(1)).pipe((0, import_operators.map)((coords) => [, coords])), mousemove$.pipe((0, import_operators.pairwise)())).pipe((0, import_operators.map)(([previousCoords, newCoords]) => {
        return [previousCoords ? getDiff(previousCoords) : previousCoords, getDiff(newCoords)];
      })).pipe((0, import_operators.filter)(([previousCoords, newCoords]) => {
        if (!previousCoords) {
          return true;
        }
      ...
      })).pipe((0, import_operators.map)(([, newCoords]) => {
        const snapGrid = getSnapGrid();
        return {
          clientX: Math.round(newCoords.clientX / snapGrid.x) * snapGrid.x,
          clientY: Math.round(newCoords.clientY / snapGrid.y) * snapGrid.y
        };
      })).pipe((0, import_operators.takeUntil)((0, import_rxjs.merge)(mouseup$, mousedown$)));
    })).pipe((0, import_operators.filter)(() => !!currentResize));
    mousedrag.pipe((0, import_operators.map)(({
      clientX,
      clientY
    }) => {
      return getNewBoundingRectangle(currentResize.startingRect, currentResize.edges, clientX, clientY);
    })).pipe((0, import_operators.filter)((newBoundingRect) => {
    ...
    }), (0, import_operators.takeUntil)(this.destroy$)).subscribe((newBoundingRect) => {
      if (currentResize && currentResize.clonedNode) {
    ...
    mousedown$.pipe((0, import_operators.map)(({
      edges
    }) => {
      return edges || {};
    }), (0, import_operators.filter)((edges) => {
      return Object.keys(edges).length > 0;
    }), (0, import_operators.takeUntil)(this.destroy$)).subscribe((edges) => {
      if (currentResize) {
        removeGhostElement();
      }
    ...
    mouseup$.pipe((0, import_operators.takeUntil)(this.destroy$)).subscribe(() => {
      if (currentResize) {
...
var PointerEventListeners = class _PointerEventListeners {
  constructor(renderer, zone) {
    this.pointerDown = new import_rxjs.Observable((observer) => {
    ...
    this.pointerMove = new import_rxjs.Observable((observer) => {
    ...
    this.pointerUp = new import_rxjs.Observable((observer) => {
...
var ResizeHandleDirective = class {
  ...
  ngOnInit() {
    this.zone.runOutsideAngular(() => {
      ...
      if (IS_TOUCH_DEVICE) {
        ...
        (0, import_rxjs.merge)(this.listenOnTheHost("touchend"), this.listenOnTheHost("touchcancel")).subscribe((event) => {
  ...
  listenOnTheHost(eventName) {
    return (0, import_rxjs.fromEvent)(this.element.nativeElement, eventName).pipe((0, import_operators.takeUntil)(this.destroy$));
  }
};
